...
public class CreateCharter_ID extends TestNGBase{
	...
	//Name: TA01_Validate_HOHUser_CreateID_MACID()
	//Description: Validate whether the user is able to create an HOH account
	//Scripter:Dhruva
	//Date/Version: 08-Oct-2015
	@Test(dataProvider="TestDataCharter", dataProviderClass = DataProviderClass.class)
	public void TA01_Validate_HOHUser_CreateID_MACID(Map<String,String> testdata) throws Exception{
		
		String password = testdata.get("UserPassword");
		String charterID=testdata.get("charterID");
		...
		Assert.assertTrue(auth.createuser_HOH(accountNum,firstname_CID,lastname_CID,Address_CID,Zipcode_CID,MAC_CID,charterID, password, email,security_question, security_answer,Mobilephone),"User is -Not able to see Successful message after Charter ID creation in create id flow");
		/*hp.logoutApplication();
		//auth.getAccountDetails(accountNum);
		auth.mailLogin(charterID,password);
	...
	//Date/Version:
	@Test(dataProvider="TestDataCharter", dataProviderClass = DataProviderClass.class)
	public void TA16_Validate_CreateID_NoDataEntry_Message(Map<String,String> testdata) throws Exception{
		...
		hp.loadCharterPage(URL);
		AuthenticationPage auth=new AuthenticationPage(driver);
		Assert.assertTrue(auth.CreateID_missedfieldsmessages(lastname_CID,Address_CID,Zipcode_CID,MAC_CID), "Next button is enabled -if data is not entered in the fields");
	}
	...
	//Name: TA77_Validate_StdUser_CreateID()
	//Description: Validate whether the user is able to create an STD user account
	//Scripter:Dhruva
	//Date/Version: 08-Oct-2015
	@Test(dataProvider="TestDataCharter", dataProviderClass = DataProviderClass.class)
	...
	//Name: TA78_Validate_HOHUser_CreateID_AccountNum()
	//Description: Validate whether the user is able to create an HOH account using Account Number
	//Scripter:Dhruva
	//Date/Version: 08-Dec-2015
	@Test(dataProvider="TestDataCharter", dataProviderClass = DataProviderClass.class)
	public void TA78_Validate_HOHUser_CreateID_AccountNum(Map<String,String> testdata) throws Exception{
		...
		hp.loadCharterPage(URL);
		AuthenticationPage auth=new AuthenticationPage(driver);
		Assert.assertTrue(auth.createuser_HOH_ACNum(accountNum,firstname_CID,lastname_CID,Address_CID,Zipcode_CID,MAC_CID,charterID, password, email,security_question, security_answer,Mobilephone),"User is -Not able to see Successful message after Charter ID creation in create id flow");
		hp.logoutApplication();
		//auth.getAccountDetails(accountNum);
/*		auth.mailLogin(charterID,password);
		auth.CID_ConfirmationMail();
